 ...
 *
 *  Disclaimer:
 *  This code is not optimised neither for performance neither for
 *maintainability. There might still be errors in the code, logic or procedure.
...
int main(int argc, char *argv[]) {
  ...
  boost::program_options::store(
      boost::program_options::parse_command_line(argc, argv, opt_description),
      var_map);
  ...
  double start_t = rand_gen.randUniform(
      overlap, sim_opt.dataset_length - integration_length - overlap);
  ...
  std::cout << "Preintegration errors over window of " << preint_meas.dt
            << " seconds:" << std::endl;
  ...
  if (test_jacobians) {
    ...
    for (size_t i = 0; i < 3; ++i) {
      ...
      for (size_t j = 0; j < data.gyr.size(); ++j)
        data_temp_bw.gyr[j].data[i] += num_quantum;
      for (size_t j = 0; j < data.acc.size(); ++j)
        data_temp_bf.acc[j].data[i] += num_quantum;

      ugpm::ImuPreintegration preint_bw(data_temp_bw, start_t, t, preint_opt,
                                        prior);
      auto temp_preint = preint_bw.get(0, 0);

      preint_num_jacobian.d_delta_R_d_bw.col(i) =
          ugpm::logMap(preint_meas.delta_R.transpose() * temp_preint.delta_R) /
          num_quantum;
      preint_num_jacobian.d_delta_v_d_bw.col(i) =
          (temp_preint.delta_v - preint_meas.delta_v) / num_quantum;
      preint_num_jacobian.d_delta_p_d_bw.col(i) =
          (temp_preint.delta_p - preint_meas.delta_p) / num_quantum;

      ugpm::ImuPreintegration preint_bf(data_temp_bf, start_t, t, preint_opt,
                                        prior);
      temp_preint = preint_bf.get(0, 0);

      preint_num_jacobian.d_delta_v_d_bf.col(i) =
          (temp_preint.delta_v - preint_meas.delta_v) / num_quantum;
      preint_num_jacobian.d_delta_p_d_bf.col(i) =
          (temp_preint.delta_p - preint_meas.delta_p) / num_quantum;
    }
    auto data_temp_dt = data;
    for (size_t j = 0; j < data.gyr.size(); ++j)
      data_temp_dt.gyr[j].t -= num_quantum;
    for (size_t j = 0; j < data.acc.size(); ++j)
      data_temp_dt.acc[j].t -= num_quantum;

    ugpm::ImuPreintegration preint_dt(data_temp_dt, start_t, t, preint_opt,
                                      prior);
    auto temp_preint = preint_dt.get(0, 0);
    preint_num_jacobian.d_delta_R_d_t =
        ugpm::logMap(preint_meas.delta_R.transpose() * temp_preint.delta_R) /
        num_quantum;
    preint_num_jacobian.d_delta_v_d_t =
        (temp_preint.delta_v - preint_meas.delta_v) / num_quantum;
    preint_num_jacobian.d_delta_p_d_t =
        (temp_preint.delta_p - preint_meas.delta_p) / num_quantum;
