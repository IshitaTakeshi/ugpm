...
int main(int argc, char *argv[]) {
  ...
  opt_description.add_options()("help,h", "Produce help message")(
      ...
      "Flag to activate the correlation of the biases");
  ...
  {
    std::cout << "\n\n\n\n" << std::endl;
    std::cout << "-- Test removing some measurements (asynchronous sensors) --"
              << std::endl;
    ...
    stop_watch.start();
    ugpm::ImuPreintegration preint(temp_data, start_t, end_t, preint_opt,
                                   prior);
    stop_watch.stop();
    stop_watch.print();
    ...
    std::vector<double> error =
        imu_sim.testPreint(start_t, end_t, preint.get());
...

  {
    ...
    std::cout << "Preintegration errors without prior over window of "
              << preint_meas.dt << " seconds:" << std::endl;
    std::cout << "  Rotation [deg] = " << error[0] * 180.0 / M_PI << std::endl;
    std::cout << "  Velocity [m/s] = " << error[1] << std::endl;
    std::cout << "  Position [m]   = " << error[2] << std::endl;
    std::cout << std::endl;
    ...
    stop_watch.start();
    ugpm::ImuPreintegration preint2(temp_data, start_t, t, preint_opt,
                                    temp_prior);
    stop_watch.stop();
    stop_watch.print();
    ...
    std::cout << "Preintegration errors with prior over window of "
              << preint_meas.dt << " seconds:" << std::endl;
    std::cout << "  Rotation [deg] = " << error[0] * 180.0 / M_PI << std::endl;
    std::cout << "  Velocity [m/s] = " << error[1] << std::endl;
    std::cout << "  Position [m]   = " << error[2] << std::endl;
