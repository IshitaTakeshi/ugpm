...
// Class to compute preintegrated measurements and potentially store the
// pre-computed meausrements
...
public:
  ...
  // times of the constructor
  PreintMeas get(const int index_1, const int index_2,
                 double acc_bias_std = 0.3, double gyr_bias_std = 0.03);

  // Overload of the get function when vector in the constructor
  PreintMeas get(const int index_1, double acc_bias_std = 0.3,
                 double gyr_bias_std = 0.03);
...
private:
  ...
  // Store the pre-computed preintegrated measurements if contructor with
  // time-stamps
...
public:
  ...
  double start() {
    ...
    if (!stopped_) {
      ...
      output = std::chrono::duration_cast<std::chrono::microseconds>(duration_)
                   .count();
    }
    last_ = temp;
    stopped_ = false;
  ...
  double getLast() {
    auto temp = std::chrono::high_resolution_clock::now();
    return std::chrono::duration_cast<std::chrono::microseconds>(temp - last_)
               .count() /
           1000.0;
  }
  ...
  double stop() {
    if (!stopped_) {
      ...
      std::cout << "WARNING: Stopping a StopWatch that is not running, "
...
// Iterative integrator of imu data (still need batch of data but the algorithm
// is mostly iterative)
class IterativeIntegrator {

public:
  IterativeIntegrator(const ImuData &imu_data, const double start_time,
    ...
    // Create a vector of timestamps of interest includung the starting
    // timestamp (and its shifted value) and the timestamps of the
    // accerlerometer data
    ...
    // Check if the data has high enough frequency, otherwise add fake
    // timestamps
    if (t.getSmallestGap() > (1.0 / min_freq)) {
      ...
      for (int i = 0; i < nb_fake; ++i)
        fake_time.push_back(offset + (i * quantum));
    ...
    std::vector<bool> interest_t;
    {
      ...
      if ((nb_infer_vec + 2) < infer_t.size())
        temp.push_back(
            std::vector<bool>(infer_t[nb_infer_vec + 2].size(), false));
    ...
    std::vector<PreintMeas> acc_time_preint =
        t.getVector(preint, nb_infer_vec + 1);
    if (!bare_) {
      std::vector<Eigen::MatrixXd> d_acc_d_bf;
      std::vector<Eigen::MatrixXd> d_acc_d_bw;
      std::vector<Eigen::VectorXd> d_acc_d_dt;
      acc_data_ = reprojectAccData(acc_time_preint, acc_data_, delta_R_dt_start,
                                   d_acc_d_bf, d_acc_d_bw, d_acc_d_dt);
...
private:
  ...
  void velPosPreintLPM(const std::vector<std::vector<double>> &t,
    ...
    for (int axis = 0; axis < 3; ++axis) {
      ...
      double temp_t_ratio =
          (start_t_ - acc_time_[ptr]) / (acc_time_[ptr + 1] - acc_time_[ptr]);
      Vec3 d_d_0_d_bw =
          d_acc_d_bw[axis].row(ptr + 1).transpose() * temp_t_ratio +
          d_acc_d_bw[axis].row(ptr).transpose() * (1 - temp_t_ratio);
      Vec3 d_d_0_d_bf =
          d_acc_d_bf[axis].row(ptr + 1).transpose() * temp_t_ratio +
      ...
      for (int i = start_index; i < time.size(); ++i) {
          ...
          while (loop) {
            if ((time.get(i) >= acc_time_[ptr]) &&
              ...
              if (ptr < (nb_acc_ - 2)) {
                ...
                Vec3 temp_d_p_d_bf =
                    dt * dt * (2.0 * d_d_0_d_bf + d_d_1_d_bf) / 6.0;
                Vec3 temp_d_p_d_bw =
                    dt * dt * (2.0 * d_d_0_d_bw + d_d_1_d_bw) / 6.0;
        ...
        preint[index.first][index.second].d_delta_v_d_t(axis) =
            (preint[index.first][index.second].delta_v(axis) - temp_d_v) /
            kNumDtJacobianDelta;
        preint[index.first][index.second].d_delta_p_d_t(axis) =
            (preint[index.first][index.second].delta_p(axis) - temp_d_p) /
            kNumDtJacobianDelta;
        preint[index.first][index.second].delta_v(axis) = temp_d_v;
        preint[index.first][index.second].delta_p(axis) = temp_d_p;
        preint[index.first][index.second].cov(3 + axis, 3 + axis) =
            temp_d_v_var;
        preint[index.first][index.second].cov(6 + axis, 6 + axis) =
            temp_d_p_var;
        ...
        preint[index.first][index.second].d_delta_v_d_bf.row(axis) =
            d_v_d_bf_backup + temp_d_v_d_bf;
        preint[index.first][index.second].d_delta_v_d_bw.row(axis) =
            d_v_d_bw_backup + temp_d_v_d_bw;
        preint[index.first][index.second].d_delta_p_d_bf.row(axis) =
            d_p_d_bf_backup + dt * d_v_d_bf_backup + temp_d_p_d_bf;
...
public:
  Se3Integrator(ImuData &imu_data, const double start_time,
    ...
    for (int i = 0; i < 6; ++i) {
      ...
      if (i < 3) {
        MatX K_int = seKernelIntegral(start_t_, state_time_, state_time_,
                                      hyper_[i].l2, hyper_[i].sf2);
        K_int_K_inv_[i].resize(nb_state_, nb_state_);
        K_int_K_inv_[i] = K_int * K_inv_[i];

        d_r_var[i] = ((-KK_inv_[i] * K).diagonal().array() + hyper_[i].sf2 +
                      hyper_[i].sz2)
                         .matrix();
        for (int j = 0; j < nb_state_; ++j) {
          if (d_r_var[i][j] <= 0)
            d_r_var[i][j] = hyper_[i].sz2;
        }
        if (correlate)
          state_std.segment(i * nb_state_, nb_state_) =
              d_r_var[i].array().sqrt().matrix();
        state_var_.segment(i * nb_state_, nb_state_) = d_r_var[i];
        d_r_var[i] = 1000.0 * d_r_var[i];
      } else {
        acc_var[i - 3] = ((-KK_inv_[i] * K).diagonal().array() + hyper_[i].sf2 +
                          hyper_[i].sz2)
                             .matrix();
        for (int j = 0; j < nb_state_; ++j) {
          if (acc_var[i - 3][j] <= 0)
            acc_var[i - 3][j] = hyper_[i].sz2;
        }
        if (correlate)
          state_std.segment(i * nb_state_, nb_state_) =
              acc_var[i - 3].array().sqrt().matrix();
    ...
    for (int i = 0; i < 3; ++i) {
      GpNormCostFunction *cost_fun =
          new GpNormCostFunction(KK_inv_[i], d_r_var[i]);
      optimisation.AddResidualBlock(cost_fun, nullptr, &(state_d_r_(0, i)));
      cost_functions.push_back(cost_fun);
    }
    RotCostFunction *rot_cost_fun =
        new RotCostFunction(&gyr_data_, gyr_time_, state_time_, K_inv_,
                            start_t_, hyper_, imu_data.gyr_var);
    optimisation.AddResidualBlock(rot_cost_fun, nullptr, &(state_d_r_(0, 0)),
                                  &(state_d_r_(0, 1)), &(state_d_r_(0, 2)));
    cost_functions.push_back(rot_cost_fun);

    AccCostFunction *acc_cost_fun =
        new AccCostFunction(&acc_data_, acc_time_, state_time_, K_inv_,
                            start_t_, hyper_, imu_data.acc_var);

    // Compute the state's corelation matrix (the LPM initialisation is good
    ...
    for (int i = 0; i < nb_state_; ++i) {
      ...
      Vec3 acc_rot_dt = delta_R_dt_start.transpose() *
                        (state_acc_.row(i).transpose() + mean_acc);
      Vec3 d_acc_d_t =
          (acc_rot_dt - (state_acc_.row(i).transpose() + mean_acc)) /
          kNumDtJacobianDelta;
  ...
  PreintMeas get(const double t) {
    ...
    for (int i = 0; i < 9; ++i) {
      // Rotation
      if (i < 3) {
        MatX ks = seKernelIntegral(start_t_, t_vec, state_time_, hyper_[i].l2,
                                   hyper_[i].sf2);
        ...
        var_vec(i) = kssInt(start_t_, t, hyper_[i].l2, hyper_[i].sf2) -
                     (ks * K_inv_[i] * (ks.transpose()))(0, 0);
        if (var_vec(i) <= 0)
          var_vec(i) = dt * dt * hyper_[i].sz2;
      ...
      // Velocity
        ...
        MatX ks = seKernelIntegral(start_t_, t_vec, state_time_, hyper_[i].l2,
                                   hyper_[i].sf2);
        MatX ks_dt = seKernelIntegralDt(start_t_, t_vec, state_time_,
                                        hyper_[i].l2, hyper_[i].sf2);
        ...
        d_v_dt(i - 3) = (ks_dt * alpha_[i])(0, 0) +
                        (ks * K_inv_[i] * d_acc_dt_[i - 3])(0, 0);
        state_ks.block(i, i * nb_state_, 1, nb_state_) = ks * K_inv_[i];

        var_vec(i) = kssInt(start_t_, t, hyper_[i].l2, hyper_[i].sf2) -
                     (ks * K_inv_[i] * (ks.transpose()))(0, 0);
        if (var_vec(i) <= 0)
          var_vec(i) = std::pow(dt, 2.0) * hyper_[i].sz2;
      }
      // Position
      else {
        MatX ks = seKernelIntegral2(start_t_, t_vec, state_time_,
                                    hyper_[i - 3].l2, hyper_[i - 3].sf2);
        MatX ks_dt = seKernelIntegral2Dt(start_t_, t_vec, state_time_,
                                         hyper_[i - 3].l2, hyper_[i - 3].sf2);
        p[i - 6] = (ks * alpha_[i - 3])(0, 0) +
                   0.5 * (t - start_t_) * (t - start_t_) * hyper_[i - 3].mean;
        d_p_dw.row(i - 6) = ks * K_inv_[i - 3] * d_acc_bw_[i - 6];
        d_p_df.row(i - 6) = ks * K_inv_[i - 3] * d_acc_bf_[i - 6];
        d_p_dt(i - 6) = (ks_dt * alpha_[i - 3])(0, 0) +
                        (ks * K_inv_[i - 3] * d_acc_dt_[i - 6])(0, 0);
        state_ks.block(i, (i - 3) * nb_state_, 1, nb_state_) =
            ks * K_inv_[i - 3];
      }
    }

    var_vec.segment<3>(6) =
        (state_ks.block(6, 3 * nb_state_, 3, 3 * nb_state_) *
         (state_var_.segment(3 * nb_state_, 3 * nb_state_).asDiagonal()) *
         (state_ks.block(6, 3 * nb_state_, 3, 3 * nb_state_).transpose()))
            .diagonal();
    for (int i = 6; i < 9; ++i) {
      if (var_vec(i) <= 0)
        var_vec(i) = 0.25 * std::pow(dt, 4.0) * hyper_[i - 3].sz2;
    }
    ...
    Mat9 temp_inv_std =
        preint.cov.diagonal().array().sqrt().inverse().matrix().asDiagonal();
    Mat9 temp_std = var_vec.array().sqrt().matrix().asDiagonal();
    Mat9 temp_d = temp_std * temp_inv_std;
    preint.cov = temp_d * preint.cov * temp_d;

    preint.cov.block<3, 3>(0, 0) =
        j_right * preint.cov.block<3, 3>(0, 0) * j_right.transpose();
...
private:
  ...
  void initialiseStateWithLPM(const ImuData &imu_data, const std::vector<double> &t_vect,
    ...
    for (int i = nb_overlap_; i < nb_state_; ++i) {
      ...
      d_r_dt_local_.col(i) =
          jacobianRighthandSO3(prev[0]) * (state_d_r_.row(i).transpose());
  ...
  void initialiseStateDiff(const ImuData &imu_data, const std::vector<double> &t_vect,
    ...
    // Prepare for the timeshift diff
    {
      ...
      std::array<Eigen::Vector3d, 3> prev = {Eigen::Vector3d::Zero(),
                                             Eigen::Vector3d::Zero()};
      for (int i = nb_overlap_; i < nb_state_; ++i) {
        for (int j = 0; j < 2; ++j) {
          Eigen::Vector3d temp_r =
              logMap(start_R.transpose() * preint.get(j, i).delta_R);
  ...
  void initialiseHyperParam(const ImuData &imu_data) {
    for (int i = 0; i < 6; ++i) {
      if (i < 3) {
        ...
        hyper_[i].sf2 = ((state_d_r_.col(i).array() - hyper_[i].mean).square())
                            .matrix()
                            .mean();
        ...
        hyper_[i].sf2 =
            ((state_acc_.col(i - 3).array() - hyper_[i].mean).square())
                .matrix()
                .mean();
...
ImuPreintegration::ImuPreintegration(
  ...
  if (!imu_data_.checkFrequency()) {
    std::cout
        << "WARNING: There might be an issue with the IMU data timestamps. "
           "This is not handled in the current version (can lead to undefined "
           "behavior or alter the performance of the preintegration)."
        << std::endl;
  }
  ...
  if (opt.quantum < 0) {
    ...
    if (opt_.type == UGPM) {
      ...
      double duration =
          (*max_element(temp_max.begin(), temp_max.end())) - start_t;
      ...
      IterativeIntegrator integrator(imu_data_, start_t, prior, infer_t,
  ...
  // If per-chunck
    ...
    for (int i = 0; i < nb_chuncks; ++i) {
      ...
      auto temp_opt = opt;
      temp_opt.quantum = -1;
      ImuPreintegration preint(temp_imu_data, chunck_start_t, temp_infer_t,
                               temp_opt, prior, rot_only, overlap);
      ...
      if (i == 0) {
        ...
        if (i != (nb_chuncks - 1)) {
          PreintMeas temp_chunck_preint = combinePreints(
              prev_chunck_preint, preint.get(infer_t.size(), 0, 0.0, 0.0));
...
ImuPreintegration::ImuPreintegration(const ImuData &imu_data,
    ...
    : ImuPreintegration(
          imu_data, start_t,
          std::vector<std::vector<double>>(1, std::vector<double>(1, infer_t)),
          opt, prior, rot_only, overlap) {
  query_type_ = kSingle;
}

// Get the preintegrated measurement as per indexed in the given inference times
// of the constructor
PreintMeas ImuPreintegration::get(const int index_1, const int index_2,
  ...
  if ((index_1 >= 0) && (index_2 >= 0) &&
    ...
    throw std::range_error("ImuPreintegration::get: Trying to get precomputed "
                           "preintegrated measurements (wrong index query?)");
  }
}

// Overload of the get function when vector in the constructor
PreintMeas ImuPreintegration::get(const int index_1, double acc_bias_std,
                                  double gyr_bias_std) {
  if (query_type_ == kVec)
    return get(0, index_1, acc_bias_std, gyr_bias_std);
  else
    throw std::range_error("ImuPreintegration::get: The type of query does not "
                           "math the type of constructor");
}

// Overload of the get function when single timestamp in the constructor
PreintMeas ImuPreintegration::get(double acc_bias_std, double gyr_bias_std) {
  if (query_type_ == kSingle)
    return get(0, 0, acc_bias_std, gyr_bias_std);
  else
    throw std::range_error("ImuPreintegration::get: The type of query does not "
                           "math the type of constructor");
}
