...
struct SineProperties3 {
  ...
  void print() const {
    ...
    std::cout << "  Frequencies [Hz]: " << freq[0] << "   " << freq[1] << "   "
              << freq[2] << "   " << std::endl;
    std::cout << "  Amplitudes [Hz]: " << amp[0] << "   " << amp[1] << "   "
              << amp[2] << "   " << std::endl;
  }
};
...
public:
  ...
  ImuSimulator(const ImuSimulatorOption &opt) {
    ...
    g_vect_ << std::sin(g_angles(0)) * std::cos(g_angles(1)),
        std::sin(g_angles(0)) * std::sin(g_angles(1)), std::cos(g_angles(0));
    g_vect_ *= opt.g_cst;
    ...
    for (int axis = 0; axis < 3; ++axis) {
      rot_sine_.amp[axis] =
          rand_gen_.randUniform(rot_amplitude_min, rot_amplitude_max);
      ...
      for (int i = 0; i < number_sines; ++i) {
        ...
        vel_sine_[i].amp[axis] =
            rand_gen_.randUniform(vel_amplitude_min, vel_amplitude_max);
        vel_sine_[i].freq[axis] =
            rand_gen_.randUniform(temp_vel_freq_min, temp_vel_freq_max);
      }
    }
    ...
    for (int i = 0; i < nb_samples; ++i) {
      ...
      for (int axis = 0; axis < 3; ++axis) {
        ...
        imu_data_.acc[i].data[axis] =
            temp_acc(axis) + rand_gen_.randGauss(0, opt.acc_std);
        imu_data_.gyr[i].t = t;
        imu_data_.gyr[i].data[axis] =
            ang_vel[axis] + rand_gen_.randGauss(0, opt.gyr_std);
      }
    }
  }
  ...
  ImuData get(double from, double to) {
    ...
    // If the query inteval does not make sense throw an exception
      ...
      throw std::invalid_argument(
          "The argument of ImuReader::Get are not consistent");
    }
  }
  ...
  std::vector<double> testPreint(double start, double end, PreintMeas preint) {
    ...
    output.push_back(logMap(preint.delta_R.transpose() * rot_start_tr *
                            eulToRotMat(getEul(end)))
                         .norm());

    double dt = end - start;
    output.push_back(
        (rot_start_tr * (stdToVec3(getVel(end)) - stdToVec3(getVel(start)) -
                         (dt * g_vect_)) -
         preint.delta_v)
            .norm());
    output.push_back(
        (rot_start_tr *
             (stdToVec3(getPos(end)) - stdToVec3(getPos(start)) -
              (dt * stdToVec3(getVel(start))) - (0.5 * dt * dt * g_vect_)) -
         preint.delta_p)
            .norm());
    return output;
  }
  ...
  Vec9 preintErrorVec(double start, double end, PreintMeas preint) {
    ...
    Vec9 residual;
    residual.segment<3>(0) = logMap(preint.delta_R.transpose() * rot_start_tr *
                                    eulToRotMat(getEul(end)));
  ...
  double getAvgVel() {
    ...
    double coeff = (imu_data_.acc.back().t - imu_data_.acc[0].t) /
                   double(nb_samples - 1.0);
    for (int i = 0; i < nb_samples; ++i) {
      double t = i * coeff;
      auto vel = getVel(t);
  ...
  double getTranslationDistance(const double from, const double to) {
    ...
    for (double t = (from + kNumericalQuantum); t < to;
         t += kNumericalQuantum) {
