...
inline PreintType strToPreintType(std::string type) {
  ...
  if (type == "lpm")
    ...
    throw std::range_error(
        "The type of preintegration method is unknown, program stopping now");
  }
  return output;
};
...
struct ImuData {
  ...
  bool checkFrequency() const {
    ...
    if (min_acc_dt < 0) {
      std::cout << "WARNING: Accelerometer data is not sorted in time"
                << std::endl;
      output = false;
    }
    if (min_gyr_dt < 0) {
    ...
    if ((range_acc_dt / max_acc_dt > 0.1) ||
        (range_gyr_dt / max_gyr_dt > 0.1)) {
      std::cout << "WARNING: Accelerometer or gyroscope data is not sampled at "
                   "a constant frequency"
                << std::endl;
      std::cout << "\tMin \\ max delta time for acc " << min_acc_dt << " \\ "
                << max_acc_dt << std::endl;
      std::cout << "\tMin \\ max delta time for gyr " << min_gyr_dt << " \\ "
                << max_gyr_dt << std::endl;
      output = false;
    }
    return output;
  ...
  void print() {
    ...
    std::cout << "  Accelerometer data (" << acc.size()
              << " samples with std = " << sqrt(acc_var) << ")" << std::endl;
    for (const auto &a : acc) {
      std::cout << "      t = " << a.t << ":    " << a.data[0] << "    "
                << a.data[1] << "    " << a.data[2] << std::endl;
    }
    std::cout << "  Gyroscope data (" << gyr.size()
              << " samples with std = " << sqrt(gyr_var) << ")" << std::endl;
    for (const auto &g : gyr) {
      std::cout << "      t = " << g.t << ":    " << g.data[0] << "    "
                << g.data[1] << "    " << g.data[2] << std::endl;
    }
  }
...
struct PreintMeas : PreintMeasBasic {
  ...
  PreintMeas(Mat3 d_R, Vec3 d_v, Vec3 d_p, double dt, double dt_sq_half,
             Mat9 cov_mat)
...
template <typename T> std::vector<int> sortIndexes(const std::vector<T> &v) {
...
public:
  ...
  std::pair<int, int> getIndexPair(const int i) const { return index_map_[i]; }
  ...
  U get(const int i, const int j, const std::vector<U> &data) const {
    ...
    throw std::range_error("SortIndexTracker2: trying to 'get' with indices "
                           "not present in the index_map");
  }
